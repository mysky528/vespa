...
function(vespa_add_target_dependency TARGET OTHER_TARGET)
    ...
    if(TARGET_TYPE STREQUAL OBJECT_LIBRARY)
        if(NOT OTHER_TARGET STREQUAL dl AND NOT OTHER_TARGET STREQUAL atomic AND NOT OTHER_TARGET STREQUAL gtest AND NOT OTHER_TARGET STREQUAL m AND NOT OTHER_TARGET STREQUAL rt AND NOT OTHER_TARGET STREQUAL "stdc++fs")
...
function(vespa_add_library TARGET)
    ...
    unset(VESPA_NON_TARGET_SOURCE_FILES)
    unset(VESPA_TARGET_SOURCE_FILES)
    if(ARG_SOURCES)
        set(SOURCE_FILES ${ARG_SOURCES})
        foreach(SOURCE_FILE IN LISTS SOURCE_FILES)
            string(REGEX REPLACE ".*TARGET_OBJECTS:.*" "TARGET" SOURCE_FILE_REPLACED ${SOURCE_FILE})
            if (NOT SOURCE_FILE_REPLACED STREQUAL "TARGET")
                list(APPEND VESPA_NON_TARGET_SOURCE_FILES ${SOURCE_FILE})
            else()
                list(APPEND VESPA_TARGET_SOURCE_FILES ${SOURCE_FILE})
            endif()
	endforeach()
    else()
        ...
        set(VESPA_NON_TARGET_SOURCE_FILES ${SOURCE_FILES})
    endif()
    ...
    if (ARG_OBJECT OR ARG_INTERFACE OR TARGET ${TARGET}_object OR NOT VESPA_NON_TARGET_SOURCE_FILES)
	unset(VESPA_ADD_IMPLICIT_OBJECT_LIBRARY)
	message("-- Not adding extra object library for target ${TARGET}")
	add_library(${TARGET} ${LINKAGE} ${LIBRARY_TYPE} ${SOURCE_FILES})
    else()
	set(VESPA_ADD_IMPLICIT_OBJECT_LIBRARY True)
	if (VESPA_TARGET_SOURCE_FILES)
	    message("-- warning, target ${TARGET} has target source files")
	endif()
	message("-- Adding extra object library for target ${TARGET}")
	add_library(${TARGET} ${LINKAGE} ${LIBRARY_TYPE} $<TARGET_OBJECTS:${TARGET}_object> ${VESPA_TARGET_SOURCE_FILES})
    endif()
    ...
    if(VESPA_ADD_IMPLICIT_OBJECT_LIBRARY)
      unset(VESPA_ADD_IMPLICIT_OBJECT_LIBRARY)
      unset(ARG_AFTER)
      vespa_add_library(${TARGET}_object OBJECT SOURCES ${VESPA_NON_TARGET_SOURCE_FILES} DEPENDS ${ARG_DEPENDS})
...
function(vespa_add_executable TARGET)
	...
	set(VESPA_ADD_IMPLICIT_OBJECT_LIBRARY True)
	message("-- Adding extra object library for target ${TARGET}")
	add_executable(${TARGET} $<TARGET_OBJECTS:${TARGET}_object> ${VESPA_TARGET_SOURCE_FILES})
    endif()
    __add_dependencies_to_target()
